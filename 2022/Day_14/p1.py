...
def solution(input):
    # iterate once to find maxes necessary to create board
    maxRow = 0
    maxCol = 0
    for line in input:
    ...
    # Add 1 to maxRow/maxCol for 1-indexed
    board = [(["."] * (maxCol + 1)) for i in range(maxRow + 1)]
    ...
    # Exit condition 1 - sand flows into the abyss
    while (sandPos[0] != maxRow):
        if (board[sandPos[0] + 1][sandPos[1]] == "."):
            sandPos[0] += 1
            ...
            restCount += 1
            # Exit condition 2 - sand flow gets blocked. (Unlikely to happen)
            if (sandPos == [0, 500]):
                break
